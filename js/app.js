...
$('#endForm').addEventListener('submit', async (e) => {
  ...
  if ($('#endAddBeverage').checked) {
    ...
    if (desc && bAmt) {
      await apiPost('createBeverage', { payload: { description: desc, amount: bAmt, date: todayStr(), linkedSessionId: id, customerName: session ? session.clientName : '', createdBy: personLabel() } });
...
function openBevModal(linkedSessionId) {
  ...
  const session = linkedSessionId ? allSessions.find((s) => s.id === linkedSessionId) : null;
  $('#bevCustomerName').value = session ? session.clientName : '';
  $('#bevModalTitle').textContent = session ? `Add a beverage — ${session.clientName} (${session.entityName})` : 'Log a beverage sale';
...
$('#bevForm').addEventListener('submit', async (e) => {
  ...
  await apiPost('createBeverage', {
    payload: {
      ...
      customerName: $('#bevCustomerName').value.trim(),
      createdBy: personLabel(),
    },
  });
  closeModal('bevModal');
...
function renderBeverages() {
  ...
  box.innerHTML = list.length ? list.map((b) => `
    <div class="bev-row">
      <div>
        <div class="bev-desc">${escapeHtml(b.description)}${b.customerName ? ` <span style="color:var(--text-dim); font-weight:400;">— for ${escapeHtml(b.customerName)}</span>` : ''}</div>
        <div class="bev-meta">${b.date}${b.linkedSessionId ? ' · mid-session' : ' · standalone sale'} · logged by ${escapeHtml(b.createdBy || '')}</div>
      </div>
